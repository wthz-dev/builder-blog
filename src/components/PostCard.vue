...
<template>
  <article class="group overflow-hidden rounded-xl border border-ink-100 bg-white shadow-soft transition hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div class="space-y-3 p-5">
        <div class="flex items-center gap-2 text-xs text-ink-500">
          ...
          <span class="inline-flex items-center gap-1">
            <span v-for="t in post.tags.slice(0, 2)" :key="t" class="rounded bg-brand-50 px-2 py-0.5 text-[11px] font-medium text-brand-700">#{{ t }}</span>
          </span>
        </div>
